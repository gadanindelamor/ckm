    ...
    with open(sql_path, encoding='utf-8', errors='replace') as f:
        for i, line in enumerate(f):
            ...
            for group in _split_value_groups(vals_str):
                ...
                if pid is not None and aid is not None and did is not None:
                    mapping[(did, pid)] = aid  # clave compuesta
...
def build_W_delta(sql_path, data, gun_disc_ids, post_to_topic, n_topics):
    ...
    if quote_tbl:
        ...
        for q in q_rows:
            ...
            if disc_id not in gun_disc_ids: continue
            # clave compuesta: source usa source_discussion_id
            src_key = (q.get('source_discussion_id'), q.get('source_post_id'))
            tgt_key = (q.get('discussion_id'), q.get('post_id'))
            if src_key[0] is None or src_key[1] is None: continue
            if tgt_key[0] is None or tgt_key[1] is None: continue
            t_src = post_to_topic.get(src_key)
            t_tgt = post_to_topic.get(tgt_key)
            n_total += 1
...
def main():
    ...
    print("=" * 60)
    print("CKM Pipeline v8g - fourforums gun control")
    print("=" * 60)
