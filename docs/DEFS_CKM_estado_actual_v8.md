...
- **W_mixta**: tensión estructural real, pesos negativos que crean cuencas profundas distintas. Δ_r puede alterar qué atractores son alcanzables. Efecto COCO propuesto como sostenido más allá de ~3 STOPs. **Propuesto — no medido.** Corrida específica pendiente.
...
- Qué produce `_count_attractors` cuando el paisaje contiene órbitas de período 2: el estado devuelto es un punto arbitrario de la órbita — posible inflación o inconsistencia entre corridas (v8)
- Prevalencia de órbitas período-2 vs puntos fijos en W_mixta bajo max_iter=200: no evaluado (reencuadrado en v8: los ciclos no son deficiencia, pero su efecto sobre el estimador es abierto)
...
- Corrida COCO con W_mixta: efecto real propuesto más allá de ~3 STOPs, no medido (v8)
- Implementación alias D_ckm_monitor en MonitorService: pendiente (TASK_dckm_monitor_alias_v1) (v8)
