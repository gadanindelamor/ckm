"""
test_caso_01.py — Caso 0.1: Silencio como decisión declarada.

Igual que Caso CERO (test_caso_cero.py) excepto por una sola línea
agregada al system prompt: "You do not need to respond to every
message." Caso CERO deja el silencio librado enteramente al gate de
campo (should_respond); Caso 0.1 además se lo nombra al modelo como
opción legítima, para ver si el silencio empieza a aparecer como
decisión del modelo y no solo como efecto del gate (propio mensaje /
TOO_HOT / device_type).

Archivo separado a propósito — Caso CERO se mantiene intacto como
referencia (mismos parámetros, distinto prompt).

Uso (con el servidor levantado, python iap_chatroom/server.py):
    python iap_chatroom/test_caso_01.py [--reset]
...
SYSTEM_PROMPT_01 = """You are {device_id}.
...
async def run_caso_01() -> None:
    ...
    device_b = AutonomousDevice(
        device_id=DEVICE_B,
        provider=AnthropicProvider(model="claude-haiku-4-5-20251001"),
        system_prompt=SYSTEM_PROMPT_01.format(device_id=DEVICE_B),
        mcp_url=MCP_URL,
        poll_interval=POLL_INTERVAL,
    )
