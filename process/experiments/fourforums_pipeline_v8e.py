"""
fourforums_pipeline_v8e.py
fix: parser streaming para mturk_2010_qr_entry — evita explosion de RAM.
...
Uso:
    python fourforums_pipeline_v8e.py --sql fourforums_no_parse_2016_05_18.sql --log run.log
    python fourforums_pipeline_v8e.py --sql ... --skip_p1p4 --log run.log
    # PEG LOG & CONSOLE
    python fourforums_pipeline_v8e.py --sql fourforums_no_parse_2016_05_18.sql --log run_v8e.log | Tee-Object -FilePath run_v8e_console.log
"""
...
def build_W_delta(sql_path, data, gun_disc_ids, post_to_topic, n_topics):
        ...
        for row in st_rows:
            if isinstance(row, dict):
                ...
                if aid is not None and sid is not None:
                    try: author_to_topic[int(aid)] = 0 if int(sid) == 2 else 1
                    except: pass
        print(f"  Autores con stance: {len(author_to_topic)}")
    else:
    ...
    if not post_to_topic:
        ...
        for pid, aid in post_author.items():
    ...
    # col_names para qr_entry (extraídas del parse CREATE TABLE)
    col_names_qr = data.get('__col_names__', {}).get('mturk_2010_qr_entry', [])
    if not col_names_qr:
        print("  WARN: col_names qr_entry no disponibles — usando índices posicionales")

    # streaming
    dispute, support, delta, n_rows, n_matched = stream_W_delta(
        sql_path, post_to_topic, col_names_qr, n_topics)
    print(f"  W dispute max={dispute.max():.3f} support max={support.max():.3f}")
...
def main():
    ...
    print("=" * 60)
    print("CKM Pipeline v8e — fourforums gun control")
    print("=" * 60)
