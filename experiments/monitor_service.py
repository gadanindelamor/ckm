...
class MonitorService:
    ...
    def evaluate(self, text: str) -> Optional[dict]:
        ...
        sigma_relaxed = self._relax(sigma_prompt.copy(), W + self._Delta)
        ...
        if self._thermostat is not None:
            ts = self._thermostat.observe(self._Delta)
            if ts.stop_applied:
                self._Delta = self._thermostat.delta_state()
            panel["thermostat"] = {
                "zone"        : ts.zone,
                "D_ckm"       : ts.D_ckm,
    ...
    def _D_ckm(self, W: np.ndarray) -> float:
        ...
        A_actual = self._count_attractors(W + Delta)
        if self._A0 is None:
            self._A0 = A_actual
