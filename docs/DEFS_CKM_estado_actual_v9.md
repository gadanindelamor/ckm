...
| **Δ_r** | D | Acumulación de pares rechazados por el Gatekeeper | Historia de perturbaciones del campo en runtime | Deformación acumulada del landscape |
...
```
D_ckm_coco    ← COCO, conduce decisiones de STOP
...
- **`D_ckm_monitor`**: `_combine_W_Delta` normaliza `Δ_r / max(Δ_r)`. STOP aplica `α·Δ_r` — escalar uniforme. La normalización cancela el escalar por construcción. Resultado: D_ckm_monitor **algebraicamente ciego** a la compresión de STOP. Verificado: valor idéntico (0.8857) en 19 evaluaciones consecutivas con Δ_r diferente en 14 órdenes de magnitud. Cancelación exacta, no aproximada.

- **`D_ckm_coco`**: opera sobre W_eff con Δ_r sin normalización interna relativa — refleja el efecto real de STOP sobre el campo navegable.

**Asimetría de calibración:** D_CKM_THRESHOLD = 0.40 calibrado con W_mixta (AMP=40). En corpus W_pos de baja densidad (WARMUP_TEXTS), el piso estructural = (174−64)/174 = 0.632 > 0.40 → el umbral no activa STOP de manera sensible. No es un bug — es asimetría documentada entre calibración (W_mixta, donde el instrumento tiene efecto real) y uso actual (W_pos, donde el piso estructural supera el umbral).
...
Trazabilidad: el panel registra `sampling_mode` y `n_warmup` por evaluación — cada decisión de STOP es trazable al modo que produjo el D_ckm que la desencadenó (REG_stochastic_eval_v2).
...
**Armstrong in-vivo** (verificado): 20/20 STOPs, α ∈ [0.05, 0.30].
...
- `frac_rec` en runtime de la corrida Armstrong = A_post/A0 en cada STOP individual; va de 0.05 a 0.68, no llega a 1.0 en ningún modo.

No son comparables directamente. "STOP no produce pérdida" se sostiene en su contexto original (barrido max_α, REG_destruccion_recuperacion_v1). La corrida Armstrong no refuta ni confirma esta propiedad para el régimen W_pos baja densidad.

**Efecto COCO por régimen de W (v8, NOTA_armstrong v2):**
- **W_pos baja densidad (WARMUP_TEXTS)**: efecto real en primeros ~3 STOPs (delta_A = +34, +31, +24). Se agota cuando Δ_r cae por debajo de ~0.05 — estado absorbente. STOP sigue aplicando pero no puede mover el landscape: el soporte de Δ_r no cambia, solo la magnitud. Estado absorbente ≠ catástrofe.
- **W_mixta — verificado (REG_wmixta_consolidado_v1, Sep 2026)**: corpus `caso09_run2` (24 textos IAP). Dos regímenes controlados — único parámetro diferente: `force_w_pos`.
  ...
  Natural (W_mixta, 56 pesos negativos): A0=26–32 (n_runs=50). 42 STOPs en 3 modos, ningún delta_A negativo, mínimo +11. Campo cicla — cruza umbral en ambas direcciones. Δ_r acotada 62–76 vs 632 sin thermostat.

  Forzado (W_pos del mismo corpus): A0=3–4. 7 STOPs en un solo modo (weighted), delta_A máximo +1. Uniform y boltzmann no cruzaron el umbral.

  **Mecanismo verificado:** la diferencia en STOPs pasa por A0 y la grilla. W_mixta produce A0 ~10× mayor (n_runs=150: 58 vs 4) — grilla fina, umbral 0.40 alcanzable. W_pos produce A0=3–4 — umbral cae entre valores que la grilla no contiene.
