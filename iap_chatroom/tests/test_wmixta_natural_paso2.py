"""
test_wmixta_natural_paso2.py

TASK_wmixta_paso2_natural_v1.md — Paso 2.

Corrida del REGIMEN NATURAL sobre los 24 textos del caso IAP 09_run2.
Solo el texto de ese caso: ni su Delta_r, ni su W, ni su historia.

CorpusService SIN force_w_pos (default): rutea por marcadores de oposicion,
con lo cual W lleva pesos negativos — 56 negativos / 66 positivos medidos en
Paso 1. Es el ciclo natural de servicios, no una condicion forzada.

Objetivo: observar como funciona COCO y como caen los STOPs en este
regimen. NO se compara con Paso 3 acá ni en su REG — eso es tarea aparte
(indicacion explicita de la TASK).
...
A0 con n_runs=50 puede diferir del de n_runs=150 — sesgo n_runs conocido
(REG_stochastic_eval_v1/v2). Se registra tal cual, sin corregir.
...
OUT_DIR = _REPO_ROOT / "process/experiments/wmixta_paso2_natural"
N_RUNS  = 50
SEED    = 42
REGIMEN = "natural"
FORCE_W_POS = False
...
def run_condition(label: str, mode: str | None) -> dict:
    ...
    W = corpus.get_W()
    assert (W < 0).sum() > 0, f"{tag}: la W natural deberia traer pesos negativos"
    ...
    for i, text in enumerate(textos):
        panel = monitor.evaluate(text, device_id=f"c09n_{i:02d}")
