"""
test_caso_02.py — Caso 0.2: Gate de dos pasos.

Mismo system prompt que Caso 0.1 ("You do not need to respond to every
message.") pero con AutonomousDevice usando el gate de dos pasos
(_gate_decision): el device decide RESPOND/SILENT *antes* de generar
texto. Si decide SILENT, no genera nada — no hay declaración de
silencio ("I'll wait.", "Listening.") en el canal ni en el corpus.

Comparación directa con Caso 0.1: misma instrucción de prompt, distinto
mecanismo de gate. El criterio diferenciador es que en Caso 0.2 los
silencios no dejan rastro textual — la ausencia de entrada es la firma
del silencio genuino.

Uso (con el servidor levantado, python iap_chatroom/server.py):
    python iap_chatroom/test_caso_02.py [--reset]
...
SYSTEM_PROMPT_02 = """You are {device_id}.
...
async def run_caso_02() -> None:
    ...
    device_b = AutonomousDevice(
        device_id=DEVICE_B,
        provider=AnthropicProvider(model="claude-haiku-4-5-20251001"),
        system_prompt=SYSTEM_PROMPT_02.format(device_id=DEVICE_B),
        mcp_url=MCP_URL,
        poll_interval=POLL_INTERVAL,
    )
