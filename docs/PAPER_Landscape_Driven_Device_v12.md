...
*v11 changes from v10_2:*
*— §1: Reencuadre N=32/N=128 — el mínimo Hopfield aplica a memorias asociativas, no a CKM.*
*— §2.3: Aiyappa et al. 2024 agregado como única arquitectura base CKM publicada; W_mixta como ferrimagnética (propuesto).*
*— §2.6: Distinción explícita herramienta/uso — CKM usa Hopfield como formalismo computacional, no para recuperación de patrones.*
*— §4.2 c(S): Frase puente — la primera línea nombra la herramienta, la segunda nombra el uso en CKM.*

*v12 changes from v11:*
*— Abstract: Δ_r → Δ en primera oración. Verificado: Δ_r es simétrica (monitor_service.py L112-114); la separación 0.78/0.02 es sobre W/Δ estática.*
*— §2.3: párrafo agregado — CKM no intenta representar contenido; P1–P4 caracterizan geometría de W como snapshot estático; P5–P7 bajo interacción acumulada.*
*— §4.1: nota sobre estructura off-diagonal de Δ_r como propiedad del campo bajo la interacción — abierta.*
*— §4.2: convergencia y atractores — Lyapunov/LaSalle (Agama Santiago 2026), _count_attractors() como estimador de |L|, verificación de simetría y diagonal cero de W_eff. D_contextual como motivación teórica de D_ckm.*
*— §5.2: nota sobre muestreo en predicciones basadas en atractores (P5).*
*— §8.5: tres dimensiones del landscape — conteo, coercividad, cuencas — distintas, no medidas del mismo escalar.*
*— §9.5: criterio de Temperatura de Néel efectiva para elicitación de pares negativos — propuesto, no verificado.*
...
Multi-agent AI interaction produces a shared knowledge field that individual device outputs and pairwise similarity scores do not capture. The Cohesive Knowledge Model (CKM) measures the structural geometry of this field through two strictly separated matrices: W, a symmetric co-occurrence matrix encoding structural memory via Hebbian dynamics, and Δ, an asymmetric directional dependency matrix encoding inference orientation from the corpus. Collapsing the two reduces attractor recovery from 0.78 to 0.02 on the CKM corpus (N=32).
...
**Scope and conditions of this work.** The operational corpus is at N = 32 nodes. Classical Hopfield networks require approximately N ≥ 128 for robust attractor diversity under Hebbian associative memory storage (capacity α_c ≈ 0.138N). CKM does not store associative memories and does not use Hebbian learning on W — the scale condition is therefore not the Hopfield associative minimum. CKM's effective minimum is an open question. At N = 32, quantitative outputs (D_ckm, c(S)) are computed correctly but interpretability is constrained by scale. The IAP
...
The closest published work to CKM's base architecture is Aiyappa, Flammini
& Ahn (2024), who model contagion dynamics over weighted belief networks
using Hopfield-like energy minimization with all-positive weights. That work
maps closely to W_base (W_pos): co-occurrence weights, Hopfield relaxation,
attractor analysis. CKM extends this base in one structural direction:
W_mixta introduces intentional negative weights at specific pairs encoding
structural opposition. This produces behavior analogous to ferrimagnetic
rather than ferromagnetic systems: W_pos drives nodes toward a single dominant
attractor; W_mixta's competing interactions produce a frustrated landscape
with multiple attractor basins. *[Proposed framing — the ferrimagnetic
characterization is an analogy supported by P4 data, not a formal derivation.
See §5.3 and REG_mapeo_magnetico_ckm_v1.md.]*
...
CKM does not attempt to represent what a knowledge field contains — it
measures how the field's geometry behaves under perturbation. The seven
predictions P1–P7 are all structural: P1–P4 characterize the landscape
geometry of W as a static corpus snapshot — hysteresis under state
traversal, cascade distributions, temporal asymmetry of k-core dynamics,
and optimal density — none is a claim about what the field represents.
P5–P7 characterize field behavior under accumulated interaction across
devices and time.
...
minimization formalism underlying CKM's cohesion measure c(S). CKM borrows
this formalism as a computational tool — not as a pattern retrieval
mechanism. The capacity for associative pattern retrieval via Hebbian learning
is latent in the architecture but is not used: Hebbian learning on W was
tested and rejected because it destroys attractor diversity. *[Verified:
recovery drops from 0.78 to 0.02. See Section 4.]* CKM's attractors are not
stored patterns — they are minima of a frustrated energy landscape shaped by
co-occurrence structure and structural tension. The inference that "CKM does
not use Hopfield for pattern retrieval, therefore the capacity does not exist"
is a logical error, not a property of the system.
...
*[Observed: the accumulated off-diagonal structure of Δ_r carries
information about which node co-activations the field persistently
rejects under the current W. These patterns are properties of the
interaction under W — not attributes of individual participants.
Open as an object of study.]*
...
energy function for the state σ under W — the formalism, borrowed from
physics as a computational tool. What CKM does with it is a distinct
question: c(S) measures the alignment between the current activation state
and the weight structure of the knowledge graph. The first clause names the
tool; the second names the use in CKM. They are different things. High c(S) indicates strong structural
...
**Relaxation and convergence.** CKM uses Hopfield relaxation as a
computational primitive throughout. With symmetric W and asynchronous
update (one node per step), V(σ) = −½σᵀWσ is a Lyapunov function:
V decreases monotonically with each update, and convergence to a fixed
point is guaranteed by the Invariance Theorem of LaSalle [Agama Santiago,
2026]. The set of fixed points — the attractors of the system — is the
maximal invariant set L in Z = {σ : dV = 0}, corresponding to the local
minima of V under W.

`_count_attractors()` is a Monte Carlo estimator of |L|: it samples
initial states σ₀ and counts distinct fixed points reached after
relaxation. Estimator quality depends on how proportionally the
sampling distribution covers the basins of attraction of each element
of L.

Both conditions required for convergence — symmetric W_eff and zero
diagonal — are satisfied by construction: Δ_r is symmetrized at
accumulation and self-pairs are excluded by the rejection-pair
generator. The guarantee extends to W_eff, not only to W.

*Qualification:* the convergence guarantee applies to asynchronous
relaxation (`hopfield.py`, `ckm_p1p4_module.py`). The synchronous
implementations (`coco.py`, `monitor_service.py`) are not covered by
this guarantee.

D_ckm operationalizes a broader concept: D_contextual, the reduction
in accessible trajectories as field cohesion accumulates —
H(trajectories_t0) − H(trajectories_tactual). The formal relation
between the two remains open. D_ckm is the discrete, computable proxy.
...
*Note on attractor-based predictions (P5):* Verified under uniform σ₀ sampling. In low-density, discrete-weight corpora, tie-breaking can dominate the count rather than basin structure [REG_situacion_medicion_ago2026_v1, REG_armstrong_boltzmann_comparison_v1]. The fourforums corpus (density >0.7) operates in a different regime. P1–P4 are not affected — they do not use _count_attractors.
