...
Escribir el script (`test_caso_09_provider_hetero_join_escalonado_2dev.py`
o el nombre que corresponda según la convención de nomenclatura ya
usada en la serie — ver archivos `test_caso_0[5-8]*.py` como referencia
de estilo) antes de correr nada. Nomenclatura: que el nombre del
archivo deje claras las variables que introduce esta corrida
(heterogeneidad de provider) y las que mantiene fijas respecto a
Caso 0.8 (join escalonado, 2 devices) — no asumir que alcanza con
numerar el caso.
...
POLL_INTERVAL = 2.0s
DELTA_T = 5s
Sin seed_human. SYSTEM_PROMPT_04A (sin explicación CKM) — igual que
0.4a/0.5/0.6/0.7/0.8.
...
Verificar antes de escribir el script que `iap_chatroom/providers/`
tiene un provider de Groq (`GroqProvider` o equivalente) y que
`GROQ_API_KEY` está configurada en `.env`. Si no existe el provider,
reportar antes de continuar — no asumir que hay que crearlo sin
consultar.
...
diagnóstico completo. Verificar que el fix sigue presente en
`services/monitor_service.py` antes de correr — no asumirlo. No
revertirlo. Si un nuevo crash con el mismo patrón de shape mismatch
aparece, reportarlo — no asumir que es el mismo bug ya corregido sin
verificar el traceback.
